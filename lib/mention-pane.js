...
let createMentionPane = ({ height, stdout, util }) => {
  ...
  // process.stdout.write 自体は意図的にパッチしない。readline はキー入力の
  // エコーや行の再描画のために process.stdout.write を直接呼び出しており、
  // これをメインログ領域への書き込みとして扱ってしまうと、キー入力のたびに
  // カーソルを強制移動させることになり readline 内部のカーソル位置管理と
  // 衝突してペイン/区切り線が消えるなどの表示崩れを引き起こす。
  // そのため console.log/console.error のみをパッチし、process.stdout.write
  // を直接呼び出すコード(core.js 側)は console.log 経由に統一する。
  let installPatches = () => {
    if (origConsoleLog) {
      return; // すでにパッチ済み
    }
    ...
    realWrite = stdout.write.bind(stdout);
