...
utility.addMessageBuffer = (data) => {
  if (utility.buffer[data.channel]) {
    utility.buffer[data.channel].push(data);

    if (utility.buffer[data.channel].length > 10) {
      utility.buffer[data.channel].shift();
    }

  } else {
    utility.buffer[data.channel] = require("fifo")();
  }
};
