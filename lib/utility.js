...
utility.showSlackBinaryImage = (path) => {
  let host = url.parse(path).host;

  if (host != "files.slack.com") {
    throw new Error("Invalid domain passed.");
  }

  let axiosHeader = {
    headers: {
      Authorization: `Bearer ${utility.token}`,
    },
    responseType: "arraybuffer"
  };

  axios.get(path, axiosHeader)
    .then(function (response) {
      // handle success
      tmp.file(function _tempFileCreated(err, path, fd, cleanupCallback) {
        fs.writeFileSync(path, new Buffer.from(response.data), "binary");
        imgcat(path, {log: true});
