  ...
  const runBackfill = async (label, manualSince) => {
    ...
    try {
      ...
      const sinceLabel = manualSince !== undefined && manualSince !== null
        ...
        : "";
      console.log(`${label} Backfilling messages for ${channelIds.length} channel(s)${sinceLabel} in background...`);
      let totalFetched = 0;
      ...
      const fetchChannel = async (channelId) => {
        ...
        try {
          ...
          console.log(`Backfill error on ${resolveChannelLabelKey(channelId)}: ${reason}`);
          return;
        }
      ...
      if (totalFetched > 0) {
        console.log(`Backfill complete: ${totalFetched} message(s) fetched.`);
