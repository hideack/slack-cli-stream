...
core.start = (commander) => {
  ...
  const twitterClient = new twitter(util.twitter);
  ...
  let collectTweets = () => {
    twitterClient.get('statuses/home_timeline', {count:30, since_id:lastestTwId}, (error,tweets,response) => {
  ...
  setInterval(collectTweets, 1000 * 10);
