...
core.start = (commander) => {
  ...
  rtm.on(RTM_EVENTS.MESSAGE, (message) => {
    ...
    let data = {
      lines: lines,
      time: time,
      channel: (util.channels[message.channel]) ? "#" + util.channels[message.channel].name : "-",
      name: (util.users[message.user]) ? util.users[message.user].name : "-"
    };
...
handler.prototype.recent = function(args, fn) {
  if (util.buffer[args[0]]) {
    ...
    messageBuffer.forEach((data) => {
      core.display(data.lines, data.time, data.channel, data.name);
    });
  }
